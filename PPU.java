...
public class PPU {
	...
	public int f_bgPatternTable; // Background Pattern Table address.
									// 0=0x0000,1=0x1000
	public int f_spPatternTable; // Sprite Pattern Table address.
									// 0=0x0000,1=0x1000
	public int f_addrInc; // PPU Address Increment. 0=1,1=32
	public int f_nTblAddress; // Name Table Address.
	...
	public int f_bgVisibility; // Background visibility. 0=Not
								// Displayed,1=displayed
	public int f_spClipping; // Sprite clipping. 0=Sprites invisible in left
								// 8-pixel column,1=No clipping
	public int f_bgClipping; // Background clipping. 0=BG invisible in left
								// 8-pixel column, 1=No clipping
	...
	public void emulateCycles() {
		...
		// int n = (!requestEndFrame && curX+cycles<341 && (scanline-20 <
		// spr0HitY || scanline-22 > spr0HitY))?cycles:1;
		for (; cycles > 0; cycles--) {
	...
	public void startVBlank() {
		...
		if (lastRenderedScanline < 239) {
			renderFramePartially(nes.gui.getScreenView().getBuffer(), lastRenderedScanline + 1,
			...
			if (f_bgVisibility == 1) {
				...
				if (!hitSpr0 && f_spVisibility == 1) {
					if (sprX[0] >= -7 && sprX[0] < 256 && sprY[0] + 1 <= (scanline - vblankAdd + 1 - 21)
							&& (sprY[0] + 1 + (f_spriteSize == 0 ? 8 : 16)) >= (scanline - vblankAdd + 1 - 21)) {
	...
	// CPU Register $2003:
	// Write the SPR-RAM address that is used for sramWrite (Register 0x2004 in
	// CPU memory map)
	...
	public short sramLoad() {
		...
		/*
		 * sramAddress++; // Increment address sramAddress%=0x100;
		 */
		return tmp;
	}
	...
	private void renderSpritesPartially(int startscan, int scancount, boolean bgPri) {
		...
		if (f_spVisibility == 1) {
			...
			for (int i = 0; i < 64; i++) {
				if (bgPriority[i] == bgPri && sprX[i] >= 0 && sprX[i] < 256 && sprY[i] + 8 >= startscan
					...
					if (f_spriteSize == 0) {
						...
						if (f_spPatternTable == 0) {
							ptTile[sprTile[i]].render(0, srcy1, 8, srcy2, sprX[i], sprY[i] + 1, buffer, sprCol[i],
									sprPalette, horiFlip[i], vertFlip[i], i, pixrendered);
						} else {
							ptTile[sprTile[i] + 256].render(0, srcy1, 8, srcy2, sprX[i], sprY[i] + 1, buffer,
									sprCol[i], sprPalette, horiFlip[i], vertFlip[i], i, pixrendered);
						}
					} else {
						// 8x16 sprites
						int top = sprTile[i];
						...
						ptTile[top + (vertFlip[i] ? 1 : 0)].render(0, srcy1, 8, srcy2, sprX[i], sprY[i] + 1, buffer,
								sprCol[i], sprPalette, horiFlip[i], vertFlip[i], i, pixrendered);
			...
			for (int ntx = 0; ntx < ntx_max; ntx++) {
				...
				for (int ty = 0; ty < 30; ty++) {
					for (int tx = 0; tx < 32; tx++) {
						...
						ptTile[baseTile + nameTable[nt].getTileIndex(tx, ty)].renderSmall(x + tx * 4, y + ty * 4,
								buffer, nameTable[nt].getAttrib(tx, ty), imgPalette);
					}
				}
	...
	public int statusRegsToInt() {
		...
		int ret = 0;
		ret = (f_nmiOnVblank) | (f_spriteSize << 1) | (f_bgPatternTable << 2) | (f_spPatternTable << 3)
				| (f_addrInc << 4) | (f_nTblAddress << 5) | (f_color << 6) | (f_spVisibility << 7)
				| (f_bgVisibility << 8) | (f_spClipping << 9) | (f_bgClipping << 10) | (f_dispType << 11);
	...
	public void reset() {
		...
		f_bgPatternTable = 0; // Background Pattern Table address.
								// 0=0x0000,1=0x1000
		f_spPatternTable = 0; // Sprite Pattern Table address. 0=0x0000,1=0x1000
		f_addrInc = 0; // PPU Address Increment. 0=1,1=32
		f_nTblAddress = 0; // Name Table Address.
							// 0=0x2000,1=0x2400,2=0x2800,3=0x2C00

		// Control Flags Register 2:
		f_color = 0; // Background color. 0=black, 1=blue, 2=green, 4=red
		f_spVisibility = 0; // Sprite visibility. 0=not displayed,1=displayed
		f_bgVisibility = 0; // Background visibility. 0=Not
							// Displayed,1=displayed
		f_spClipping = 0; // Sprite clipping. 0=Sprites invisible in left
							// 8-pixel column,1=No clipping
		f_bgClipping = 0; // Background clipping. 0=BG invisible in left 8-pixel
							// column, 1=No clipping

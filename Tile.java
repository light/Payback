...
public class Tile {
	...
	public void renderSimple(int dx, int dy, Graphics g, boolean mirrorV, boolean mirrorH) {

		for (int i = 0; i != 8; i++) {
			for (int j = 0; j != 8; j++) {
				int index = j * 8 + i;
				if (index < 64) {
					int paletteIndex = pix[index];
					if (paletteIndex != 0) {
						int xx = mirrorH ? 7 - i : i;
						int yy = mirrorV ? 7 - j : j;
						g.drawLine(dx + xx, dy + yy, dx + xx, dy + yy);
					}
				}
			}
		}
	...
	public void render(int srcx1, int srcy1, int srcx2, int srcy2, int dx, int dy, int[] fBuffer, int palAdd,
			int[] palette, boolean flipHorizontal, boolean flipVertical, int pri, int[] priTable) {
		...
		if (!flipHorizontal && !flipVertical) {
			...
			for (y = 0; y < 8; y++) {
				for (x = 0; x < 8; x++) {
					if (x >= srcx1 && x < srcx2 && y >= srcy1 && y < srcy2 && tIndex < 64) {
						palIndex = pix[tIndex];
